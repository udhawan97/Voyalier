...
Fetching the details of a page you saved as a research resource is gated twice over. Saving a link or a file contacts nothing at all; it is stored on your device and that is the whole of it. Fetching what the page says needs a setting you turn on first — "allow fetching page details", off by default and reversible in one click — and until you do, no fetch happens and the button is not there to press. When you do press it, Voyalier requests the address you saved, so that site sees the request the same way it would see your browser, and stores a dated snapshot of the page's title, description, and readable text with script and stylesheet content removed. That stored text is what makes the page readable offline later; unlike your notes and confirmations it is not encrypted at rest, because it is public web material that has to stay searchable.
