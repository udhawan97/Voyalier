...
| 🛂  | **Prepare the visa** — set your passport and work the steps for a curated route                    | A step-by-step application cockpit where every requirement is a link to the authority and every sentence Voyalier writes is a translation or a caution |
...
| 🔍  | **Find anything** — search documents, confirmed facts, notes, saved places, and manual plans       | Local workspace results with trip/source provenance and transparent occurrence-based ranking                                                           |
...
| **Visa preparation** | Quote a destination's published entry path with its source and the date it was read; walk a curated route step by step with plain-language explanations, per-document cautions, and official links; persist ticks and sealed notes per trip. Canada is curated first; nothing is ever asserted     |
...
| **Offline packs**    | Browse a validated catalog and download per-trip Overture + Wikivoyage packs from GitHub Releases with separate layer licenses; Nashville, Kyoto, Tokyo, and Paris also have bounded verified PMTiles targets                                                                                      |
...
| **Search**           | Scan every trip's source documents, confirmed facts, notes, saved places, and manual plans locally, returning bounded snippets and provenance with deterministic ranking; pending candidates remain excluded                                                                                       |
...
- **More offline city coverage** — Nashville, Kyoto, Tokyo, and Paris now use the bounded multi-map publishing path; additional catalog cities remain incremental work
- **More sourced readiness** — only where licensing, retrieval, and authority boundaries stay honest. (Visa preparation shipped in 0.6.0 for Canada: pointers and plain-language steps, never determinations. More destinations are curation work.)
