...
/// `(kind, decoded body)` or `None` for a non-text part (image/attachment).
fn best_part(entity: &str) -> Option<(DocumentKind, String)> {
...
fn decode_rfc2047(value: &str) -> String {
    ...
    while let Some(start) = rest.find("=?") {
        ...
        match decoded {
            Some(text) if charset.to_ascii_uppercase().starts_with("UTF-8") => out.push_str(&text),
