...
`packages/contracts` is hand-written TypeScript with no codegen. `AppGateway` is the one interface; a new method must land in every one of: `AppService`, the Axum route, the Tauri command, `contracts/src/index.ts`, `contracts/src/mock.ts`, and both `gateway/http.ts` and `gateway/tauri.ts`. TypeScript catches the last three, and `packages/contracts/parity/routes.json` catches the transports: it declares each method's HTTP verb, HTTP path, Tauri command, and — since ADR-0012 — its request `payload` (the Tauri envelope keys, the HTTP body, the query keys). It is asserted from `apps/web/src/routeParity.test.ts` (both gateways, including the keys each actually puts on the wire, plus the two desktop-only bridges — `updater/tauriUpdater.ts` and `backup/tauriBackup.ts` — against `desktopOnly`), `voyalier-server` (the router's routes equal the manifest's, in both directions; each handler's `Json<T>` and `Query<T>` fields equal the declared keys; and the crate never names a desktop-only command), and `voyalier-desktop` (`generate_handler!` registers every declared command and nothing more, and each command's input struct reads the declared keys). A payload row is `"input"` when the gateway forwards a whole shared `voyalier-core` type, a literal key list when it hand-builds one, or `{"whole": [...]}` when it forwards whole onto a locally re-declared struct. Drift either way fails the gate — a method wired without a manifest row, or a row nothing is wired to. It is hand-maintained — never regenerate it from a gateway.
...
- `scripts/check.sh integration` boots the real loopback server against temporary data for `gateway.live.test.ts`, then runs Playwright through the real Axum + Vite stack with a disposable SQLite workspace. Route names, verbs, path shapes, and request-payload key names are caught separately by the `parity/routes.json` guard (ADR-0012). What that guard does not cover is the field names inside a forwarded shared type — a row saying `"input"` asserts the forward happened, not that the TypeScript interface and the Rust struct agree field by field. ADR-0023 adds shared wire fixtures for the trip/import/amendment/restore/plan journey: `desktopJourneyWire.test.ts` pins typed gateway envelopes and the desktop suite dispatches them through Tauri MockRuntime against disposable SQLite. This covers the selected fields only; installed-webview serialization remains a separate future layer.
