...
has no data key and every sealed row is undecryptable. A backup is only _real_ if
...
4. **Full user-facing slice**, shipped as a **desktop-only capability** following
   ...
   _not_ the 6-place gateway contract). `packages/contracts`, `mock.ts`, and the
...
- **Restore always lands in keychain mode.** Apply installs the carried key as
  ...
  (existing feature). The backup passphrase is only the _file's_ lock.
...
- [ ] `BackupPanel` (in Settings, beside Updates/Encryption): an **Export** action (passphrase entered
      **twice** + a blunt _"There is no recovery if you lose this passphrase"_ warning mirroring the vault
