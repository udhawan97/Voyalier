...
- **Active:** a data key is available from the OS keychain, or the traveler has
  ...
  (ADR-0016), so two data directories never share a key — and never delete each
...
- [ADR-0016](ADR-0016-the-vault-key-belongs-to-its-database.md) — the vault key belongs to its database
